...
| **WSL2 + AMD GPU** | ✅ Mesa D3D12 | ✅ WebGL / ⚠️ Vulkan | ⚠️ VA-API (Mesa D3D12) | `--encoder amd-wsl`。`svc-selkies` が起動時に実際の `h264_vaapi` エンコードを試し、Windows ドライバの D3D12 Video Encode が動けば分離 FFmpeg エンコーダーを使用、動かなければ警告を出して x264 にフォールバック |
...
- `nvidia-wsl` のハードウェアエンコード（NVENC）は OpenGL とは独立して Pixelflux から使用。WSL2 には NVIDIA の DRM レンダーノードが存在せず（仮想 `faux_driver`/vgem ノードのみ）、Pixelflux 2.0 はレンダーノードの sysfs ドライバ名だけで NVENC を判定するうえ、WSL の `libcuda.so.1` には Pixelflux が起動時に解決する CUDA/EGL 相互運用シンボルが 2 つ欠けているため、従来は `svc-selkies` が NVENC と表示しながら実際には CPU x264 に無音でフォールバックしていた。現在は `svc-selkies` が CUDA/NvEncodeAPI ランタイムを検証し、`pixelflux-nvenc-wsl.so`（[ソース](files/ubuntu-root/usr/local/src/pixelflux-nvenc-wsl.c)）を Selkies プロセスへ preload し、エンコーダーを `/dev/dxg` に向けて Pixelflux の読み戻し経路（Mesa D3D12 で描画 → 読み戻し → CUDA へ転送 → NVENC で圧縮）を選択させる。ブラウザに保存された `use_cpu=true` で CPU に戻らないよう `SELKIES_FORCE_HARDWARE_ENCODING=1` も設定する。確認は `./check-wsl-gpu.sh`、または `docker logs linuxserver-kde-$(whoami) | grep -E 'NVENC|Mode: H264'`（`NVENC Encoder initialized successfully` と `Mode: H264 (NVENC)` が出る）、およびビューア接続中にコンテナ内で `nvidia-smi --query-gpu=utilization.encoder --format=csv`（`nvidia-smi encodersessions` は WSL でハングする）。ホストブラウザには `prefer-hardware` と NVENC に合わせたコーデック文字列（`avc1.640034`: High Profile / Level 5.2）を配布する
...
- Adapter選択とD3D12 OpenGLは3つのWSL GPU vendorで共通で、`MESA_D3D12_DEFAULT_ADAPTER_NAME`の既定を`Intel`、`NVIDIA`、`Radeon`に分ける。一方、固定版`/opt/wsl-vaapi-legacy` VA-API driverと直列化した外部FFmpeg Encoderは`intel-wsl`専用のままにする。Selkies配信ではNVIDIAはNVENC、AMDはsystem D3D12 VA-API driverを使うため、Intel旧video stackをNVIDIA/AMDへ流用しない。 `amd-wsl` は `intel-wsl` と同じ分離 FFmpeg 読み戻しエンコーダー（ベンダー非依存）を使うが、Intel 固定版 Mesa と VA 同期シムは使わず、システム Mesa の d3d12 VA-API ドライバで動かす。WSL では同一プロセス内ゼロコピー VA 経路が D3D12 GL バッファを取り込めず、失敗しても CPU に落ちるだけなので、`svc-selkies` はまずコンテナの `MESA_D3D12_DEFAULT_ADAPTER_NAME` で `/dev/dri/card0` に対し 30 フレームの `h264_vaapi` エンコードを実行し、符号化パケットが返った場合のみ分離エンコーダー（VBR 4 Mbps、ホイール側上限 8 Mbps、paint-over 無効）を有効にする。返らなければ `falling back to CPU x264` を記録する。`./check-wsl-gpu.sh` はコンテナからアダプタとドライバの選択を読み取り、両ケースを報告する。ホストブラウザへのコーデック文字列は Mesa の d3d12 ドライバが SPS を書くため Intel と同じ `avc1.640C29`。AMD の WSL 実機ではまだ検証していない。
- `startwm_wayland.sh` が `plasmashell` を再起動する場合（バックエンド切替時、または `s6-svc -r /run/service/svc-selkies` や WSL の GPU デバイス消失に伴う KWin 再起動後）は、稼働中セッションプロセス（`plasma_session`、無ければ `kded6`、`kwin_wayland`）の環境変数で起動する。startplasma は `XDG_CONFIG_DIRS`（`~/.config/kdedefaults`、Kubuntu 既定設定）、`XDG_MENU_PREFIX`、`LC_*` を自分の子プロセスにしか渡さないため、素のサービス環境で起動した shell は kded6/KWin と追跡する `mimeapps.list` の集合が食い違い、KService が共有 KSycoca キャッシュを毎回「古い」と判定して plasmashell（約 60% CPU）と kwin_wayland（約 30%）がアイドル状態でも交互に再構築し続け、kdedefaults も無視されていた。この負荷を見たら `~/.cache/ksycoca6_*` が数秒ごとに書き換わっていないか、plasmashell と kded6 の `/proc/<pid>/environ` の差分を確認する。
...
- `nvidia`のXvfb profileでは、X11 sessionがデスクトップから起動するアプリにGPU OpenGLを継承させるため`plasmashell`をZinkで再起動する。この再起動（およびkwin_x11/plasmashellの監視による再起動）は、稼働中のPlasma sessionプロセスの環境変数を引き継いで行う。startplasma-x11は自身の子プロセスにだけ`~/.config/kdedefaults`を含む`XDG_CONFIG_DIRS`を渡すため、これがないと再起動後のshellは選択したグローバルテーマを無視し（汎用ランチャーアイコン、Breeze Light配色）、同じ不完全な環境を全アプリへ渡していた。Zinkのshellはさらに`QT_XCB_GL_INTEGRATION=xcb_egl`を使う。GLXではタスクマネージャのウィンドウサムネイルが`GLX_EXT_texture_from_pixmap`を経由し、そのコピーがNVIDIA Vulkanドライバ内でクラッシュしていた（タスクボタンにポインタを乗せた直後にplasmashellがSIGSEGV）。EGLではサムネイルがウィンドウアイコンへフォールバックし、描画はZinkのまま維持される。plasmashellの監視はKCrash/drkonqi待ちで停止（またはdefunct）したshellも停止扱いにして再起動し、Zink再起動は`plasmashell --replace`の競合（shellが1つも残らず黒画面になる）を避けて旧shellの終了を待ってから起動する。
