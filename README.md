...
| **WSL2 + AMD GPU** | ✅ Mesa D3D12 | ✅ WebGL / ⚠️ Vulkan | ⚠️ VA-API (Mesa D3D12) | `--encoder amd-wsl`; `svc-selkies` runs a real `h264_vaapi` encode at startup and uses the isolated FFmpeg encoder when the Windows driver's D3D12 Video Encode works, else x264 with a logged warning |
...
- `nvidia-wsl` encodes with NVENC, independently from the OpenGL rendering path. WSL2 has no NVIDIA DRM render node (only the virtual `faux_driver`/vgem node), yet Pixelflux 2.0 decides on NVENC solely from the render node's sysfs driver name, and the WSL `libcuda.so.1` lacks two CUDA/EGL interop symbols Pixelflux resolves up front; unaided it silently fell back to CPU x264 while `svc-selkies` claimed NVENC. `svc-selkies` now verifies the CUDA/NvEncodeAPI runtime, preloads `pixelflux-nvenc-wsl.so` ([source](files/ubuntu-root/usr/local/src/pixelflux-nvenc-wsl.c)) into the Selkies process and points the encoder at `/dev/dxg`, which selects Pixelflux's readback path: Mesa D3D12 renders, the frame is read back and uploaded to CUDA, NVENC compresses. It also sets `SELKIES_FORCE_HARDWARE_ENCODING=1` so a browser whose saved settings still say `use_cpu=true` cannot silently keep the stream on the CPU. Verify with `./check-wsl-gpu.sh` or `docker logs linuxserver-kde-$(whoami) | grep -E 'NVENC|Mode: H264'` (expect `NVENC Encoder initialized successfully` and `Mode: H264 (NVENC)`), and `nvidia-smi --query-gpu=utilization.encoder --format=csv` inside the container while a viewer is connected (`nvidia-smi encodersessions` hangs on WSL). The host browser gets `prefer-hardware` with an NVENC-matched codec string (`avc1.640034`: High Profile, Level 5.2)
...
- Adapter selection and D3D12 OpenGL are common to all three WSL GPU vendors: `MESA_D3D12_DEFAULT_ADAPTER_NAME` defaults to `Intel`, `NVIDIA`, or `Radeon`. The pinned `/opt/wsl-vaapi-legacy` VA-API driver and serialized external FFmpeg encoder remain exclusive to `intel-wsl`; NVIDIA uses NVENC for the Selkies stream, while AMD uses the system D3D12 VA-API driver. Do not copy the Intel legacy video stack into NVIDIA/AMD profiles. `amd-wsl` shares the isolated FFmpeg readback encoder with `intel-wsl` (it is vendor neutral), but with the system Mesa d3d12 VA-API driver and without Intel's pinned Mesa build or VA synchronization shim. The same-process zero-copy VA path cannot import the D3D12 GL buffers on WSL, and a same-process failure only falls back to CPU, so `svc-selkies` first runs a 30-frame `h264_vaapi` encode on `/dev/dri/card0` with the container's `MESA_D3D12_DEFAULT_ADAPTER_NAME` and enables the isolated encoder (VBR 4 Mbps, 8 Mbps ceiling from the wheel, paint-over disabled) only when coded packets come back; otherwise it logs `falling back to CPU x264`. `./check-wsl-gpu.sh` reads the adapter and driver choice from the container and reports both cases. The host browser gets the same `avc1.640C29` codec string as Intel because Mesa's d3d12 driver writes the SPS. AMD WSL has not been exercised on real hardware here.
- `startwm_wayland.sh` relaunches `plasmashell` (backend override, or after a KWin/compositor restart such as `s6-svc -r /run/service/svc-selkies` or a WSL GPU device loss) with the environment of a live session process (`plasma_session`, else `kded6`, else `kwin_wayland`). startplasma hands `XDG_CONFIG_DIRS` (`~/.config/kdedefaults`, Kubuntu defaults), `XDG_MENU_PREFIX` and `LC_*` only to its own children; a shell started with the bare service environment tracked a different set of `mimeapps.list` files than kded6/KWin, so KService judged the shared KSycoca cache stale on every lookup and plasmashell (about 60% CPU) and kwin_wayland (about 30%) rebuilt it in turn on an idle desktop, while the shell also ignored kdedefaults. If you see that load, check `~/.cache/ksycoca6_*` being rewritten every few seconds and compare `/proc/<pid>/environ` of plasmashell and kded6.
