...
- Ubuntu 24.04 includes a private D3D12-only Mesa 25.2.8 `libgallium` rebuilt with valid no-op values for disabled blend, depth and stencil state. The distro implementation left those enum members at zero; the tested Intel WSL UMD rejected KWin's graphics pipeline with `E_INVALIDARG`, and Mesa terminated `kwin_x11`, removing the complete title bar and its minimize/maximize/close buttons. The private library is loaded only by WSL D3D12 sessions, so distro Mesa for native Linux and other backends is unchanged. KWin remains on OpenGL/D3D12 and no software renderer is selected. KWin 5 can also create its initial decoration textures before the D3D12 GLX context has settled; the startup script performs an immediate compositor suspend/resume after the KWin D-Bus service is ready to rebuild that OpenGL scene. It does not select or leave behind a software compositor. An X11-session watchdog restarts KWin and repeats the scene initialization after a later host GPU reset.
